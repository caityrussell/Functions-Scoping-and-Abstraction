...
def view_flights(flights):
    ...
    for flight in flights:
        ...
        print(f"{flight['flight_no']:<8} {flight['source']:<6} {flight['dest']:<6} {flight['seats']:>6} ${flight['price']:.2f}")
    print("-" * 43)
...
def book_flight(passenger_name, file_name, flights, bookings):
    ...
    seats_input = input(f"How many seats would you like to book on {flight_no}? ")

    if not seats_input.isdigit():
        print("Invalid number of seats.")
        return
    seats_needed = int(seats_input)
